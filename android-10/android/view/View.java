...
public class View implements Drawable.Callback, KeyEvent.Callback, AccessibilityEventSource {
    ...
    */
    ...
    /**
     * Indicates(显示,指定);temporarily(临时的);detached(分开)
    ...
    */
    ...
    /**
     * <li>这个方法在ViewGroup的addViewInner()方法和updateViewLayout()中被调用</li>
     ...
     */
    /*我们说过，当UI框架开始绘制时，皆是从ViewRoot.java类中的performTraversals()开始绘制的。
    ...
    public final void measure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        //mPrivateFlags这个标记就是控制View是否该重新绘制是否 该布局了等等
        if ((mPrivateFlags & FORCE_LAYOUT) == FORCE_LAYOUT ||
                widthMeasureSpec != mOldWidthMeasureSpec ||
                heightMeasureSpec != mOldHeightMeasureSpec) {

            // first clears the measured dimension flag
            //清除MEASURED_DIMENSION_SET标记，若不清楚该标记，那么就标识现在还不能重新绘制，等会调用onMeasure方法就会出错
            ...
            //下一步是layout了，添加LAYOUT_REQUIRED标记 ，表示着现在可以layout了
            mPrivateFlags |= LAYOUT_REQUIRED;
        }
        //保存值，以便后面判断控件的大小是否发生改变
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        //设置控件测量的宽和高，getDefaultSize()通过MeasureSpec的不同的模式(UNSPECIFIED,AT_MOST,EXACTLY)去获取到实际大小
        //若该View是整个程序的根节点，那么他的widthMesureSpec的Mode是WRAP_Content 
     ...
     * 
     ...
     (只能是其一)要求。 它有三种模式：
     <li>①、UNSPECIFIED(未指定)，父元素部队自元素施加任何束缚，子元素可以得到任意想要的大小；</li>
     <li>②、EXACTLY(完全)，父元素决定自元素的确切大小，子元素将被限定在给定的边界里而忽略它本身大小；</li>
     <li>③、AT_MOST(至多)，子元素至多达到指定大小的值。</li>
    ...
    public static class MeasureSpec {
        ...
        /**
         * Creates a measure specification based on the supplied size and mode.<br>
         * 创建一个整形值，其高两位代表mode类型，其余30位代表长或宽的实际值。可以是WRAP_CONTENT、MATCH_PARENT或具体大小exactly size<br>
