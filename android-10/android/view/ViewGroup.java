...
public abstract class ViewGroup extends View implements ViewParent, ViewManager {
    ...
    public boolean dispatchTouchEvent(MotionEvent ev) {
        ...
        //得到触摸点的x,y坐标		
        //ev.getX和ev.getY得到是触摸位置离该控件边缘的位置
        final int action = ev.getAction();
        final float xf = ev.getX();
        final float yf = ev.getY();
        //坐标转化，界面坐标和视图坐标?
        /*
        界面坐标:是在手机屏幕的范围类，它的x,y坐标的范围是在手机屏幕的范围类
        视图坐标:视图坐标是无限大的，最顶层容器是多大，就是多大，例如:宽为320px的手机
        ViewPager的Adapter里面设置了3个界面，那么视图坐标的范围就是320*3,而界面坐标的范围是320
        */
        ...
        /*
            解释disallowIntercept这个变量，我先引入三个角色(祖父，父亲，儿子)，当前的ViewGroup表示父亲这个角色
            若disallowIntercept表示false，那么event传递的过程为祖父->父亲->儿子。
            若disallowIntercept表示true，那么event传递的过程为祖父->儿子，不经过父亲。
        */
        /*  设置disallowIntercept的值
        ...
        */
        //mGroupFlags这个标记会通过requestDisallowInterceptTouchEvent方法去修改FLAG_DISALLOW_INTERCEPT从而改变值，
        //向ListView在
        boolean disallowIntercept = (mGroupFlags & FLAG_DISALLOW_INTERCEPT) != 0;
        //mMotionTarget只会在DOWN的时候去记录，该变量是记录的是消费该事件的控 件
        if (action == MotionEvent.ACTION_DOWN) {
            ...
            // intercept
             /*
                在ViewPager,ListView等可滑动的控件中onInterceptTouchEvent(ev)默认返回的是true,
                其他的父容器都是返回的false.如果下面的if条件进不去,那么mMotionTarget就会为空,从而导致
                ViewGroup不把事件传递下去,根据下面判断可知决定子控件是否能接受到事件就是处决于这两个变量
            */
            if (disallowIntercept || !onInterceptTouchEvent(ev)) {
                ...
                //mChildren里面装的是该ViewGroup容器里面装的所有子控件
                final View[] children = mChildren;
                //记录着子控件的总数
                final int count = mChildrenCount;
                //倒叙遍历，也就是说先获取到最后添加进去的子控件
                for (int i = count - 1; i >= 0; i--) {
                    final View child = children[i];
                    //判断当前的子View是否可见，不可见就不处理
                    if ((child.mViewFlags & VISIBILITY_MASK) == VISIBLE
                            || child.getAnimation() != null) {
                        //传递一个Rect对象,调用getHitRect方法后,该对象里面就有值了,存储着该View的坐标
                        child.getHitRect(frame);
                        //判断当前ViewGroup容器是否包含该子View,若在则返回true,否则为false
                        if (frame.contains(scrolledXInt, scrolledYInt)) {
                            // offset the event to the view's coordinate system
                            //因为前面把界面坐标转化为了视图左边来进行一系列的运算,现在要把坐标转回来
                            //child.mLeft表示该子View的边缘距离父容器的边缘的距离
                            final float xc = scrolledXFloat - child.mLeft;
                            final float yc = scrolledYFloat - child.mTop;
                            ev.setLocation(xc, yc);
                            child.mPrivateFlags &= ~CANCEL_NEXT_UP_EVENT;
                            //dispatchTouchEvent的返回值跟控件类型，onTouch方法，onTouchEvent方法的返回值有关
                            /*
        ...
        * 代码走到这里说明
        * 1.说明没有子控件去处理这个事件
        * 2.说明Viewgroup拦截到了事件，但自己没处理
        * */
        //如果这次是ACTION_UP事件还是ACTION_CANCEL事件，那么isUpOrCancel就为true
        ...
        if (isUpOrCancel) {
            ...
            //这个变量会影响下一次事件,也就是move和up的事件处理
            //FLAG_DISALLOW_INTERCEPT : 0111 1111 1111 1111 1111
            //mGroupFlags :             0000 0000 0000 0000 0000
            mGroupFlags &= ~FLAG_DISALLOW_INTERCEPT; //计算后为0
        }
        
        // The event wasn't an ACTION_DOWN, dispatch it to our target if
        // we have one.
        final View target = mMotionTarget;
        //target==null 说明事件没有被子控件处理掉,则让该ViewGroup去处理
        if (target == null) {
    ...
    private void addViewInner(View child, int index, LayoutParams params,
        ...
        //判断params对象是否为null  
        if (!checkLayoutParams(params)) {
            //如果params对象是为null，重新构造个LayoutParams对象
            params = generateLayoutParams(params);
        }
        //默认preventRequestLayout值为false  
    ...
    /**
     * <li>根据AttributeSet得到一个LayoutParams的实例</li>
